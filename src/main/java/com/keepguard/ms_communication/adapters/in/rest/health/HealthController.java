...
/**
 * Controller para verificação de saúde do microserviço ms-communication.
 * 
...
public class HealthController implements HealthIndicator {
    ...
    @GetMapping
    @Operation(
        summary = "Health check do ms-communication",
        description = "Verifica se o serviço ms-communication está funcionando corretamente, incluindo conexão com banco de dados."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<Map<String, Object>> healthCheck() {
        log.debug("Health check solicitado para ms-communication");
        ...
        response.put("service", "ms-communication");
        ...
        if (isHealthy && dbHealthy) {
            log.debug("Health check ms-communication: OK");
            return ResponseEntity.ok(response);
        } else {
            log.warn("Health check ms-communication: FALHOU - Database: {}", dbHealthy ? "UP" : "DOWN");
    ...
    public Health health() {
        ...
        if (dbHealthy) {
            return Health.up()
                .withDetail("service", "ms-communication")
